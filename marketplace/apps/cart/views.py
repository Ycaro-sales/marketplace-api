...
class CartItemViewSet(generics.RetrieveDestroyAPIView):
    queryset = CartItem.objects.all()
    serializer_class = CartItemSerializer
    # permission_classes = [permissions.IsAuthenticated, isOwner]

    def perform_create(self, serializer, cart_product_id):
        if CartItem.objects.all().filter(id=cart_product_id).exists():
